...
The `ItemListing` API provides a set of methods to control the listing appearance, showing and hiding columns, sorting and refreshing the item set, and so on.
...
To select which properties are to be rendered as listing columns and to declare the columns order, the `PropertyListing` builder API provides specialized version of the `build()` method, which accept an array or an Iterable of `Property` to declare the visible columns and their order.

[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/examples/ExampleListing.java[tag=propertyListing2,indent=0]
----
<1> Set the listing visible columns: the `DESCRIPTION` property first and then the `ID` property
...
NOTE: Of course, a `VirtualProperty` can be used in a listing component not only to provide a `Component` value, but any other supported type.
