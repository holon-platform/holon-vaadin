...
All the builders can be obtained by using the link:{apidir}/com/holonplatform/vaadin/components/Components.html[Components^] interface, which is organized in  sub-interfaces by components kind.
...
Input components support the _read-only_ (to prevent the user from changing the value) and _required_ (to show a suitable indicator along with the field) modes.
...
=== Input with value converter

The `Input` interface makes available a set of methods to create an `Input` from another `Input` with a different value type, providing a suitable _converter_ to perform value conversions.
...
The `Input` interface also supports the creation of an `Input` component with value conversion using a Vaadin `HasValue` Component instead of another `Input`.

[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/examples/ExampleInput.java[tag=input7,indent=0]
----
<1> Create a `Boolean` type Input from an `Integer` type Input, providing the value `Converter`
<2> Create a `Boolean` type Input from an `Integer` type Input, using a default `PropertyValueConverter`
<3> Create a `Long` type Input from a `TextField`, providing the Long-to-String and String-to-Long value `Converter`
...
A `ValidatableInput` can be obtained from an existing `Input` or `HasValue` component instance or by using the appropriate builder.
...
The _selectable_ input builders allow to explicitly set the selection item *captions* and *icons*. As for the items caption, the Holon platform *localization* architecture is supported, and the builders provide methods to specify the caption using a `Localizable` or a _message code_.
...
The Holon platform `Property` model is fully supported by the selectable input builder, which allows to create selectable inputs using `PropertyBox` type items and a specific `Property` as selection property (typically, the property which acts as item id).
...
When the Holon platform `Property` model is used for selectable Inputs and the selection value type is not a `PropertyBox`, i.e. the selection items are not of the same type of the Input value, a conversion function is required to obtain the `PropertyBox` item from a selection value when the value is setted in the Input component, either using `setValue(...)` or `select(...)`.

For example, if a `String` type `Property` is declared as selection property, the `Input` value will be the the value of selection property (of `String` type), while the selection items are `PropertyBox`.
