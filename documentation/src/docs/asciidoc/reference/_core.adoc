...
<4> Add an overall (group) `Validator` which reads the `ID` property value from the group `PropertyBox` value and checks it is greater than 0
...
[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/examples/ExampleInputGroup.java[tag=form21,indent=0]
----
...
<2> Set the `ViewComponent` value
<3> Get the `ViewComponent` value
...
<5> Get the form value as a `PropertyBox`
