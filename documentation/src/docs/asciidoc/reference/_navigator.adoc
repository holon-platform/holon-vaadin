...
The `holon-vaadin-navigator` artifact makes available an extension of the default Vaadin *view navigator*, represented by the link:{apidir}/com/holonplatform/vaadin/navigator/ViewNavigator.html[ViewNavigator^] API.
...
* Support for link:holon-core.html#Context[Context resources^] injection in `View` instances.

[[ViewParameters]]
=== View parameters injection
...
<2> Provide the `View` content using the `getViewComponent()` method. In this example, a different UI component is provided according to the device type, checking if the user is running the application in a _mobile_ device

=== ViewNavigator Configuration
...
* A `ViewProvider` to provide the `View` instances by view _name_
...
By default, the `ViewNavigator` tracks views navigation history, to made available navigation operations such `navigateBack()` to navigate to the previous view.
...
The `ViewNavigator` API supports displaying a `View` content in an application `Window`. Instead of using the configured _view display_ component, the `View` contents will be displayed in a _popup dialog_, using the `com.vaadin.ui.Window` component.
...
[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/examples/ExampleView.java[tag=window1,indent=0]
----
<1> Opens the View named `myView` in a `Window`

==== `@WindowView` annotation

The link:{apidir}/com/holonplatform/vaadin/navigator/annotations/WindowView.html[WindowView^] annotation can be used to declare that a `View` should always be displayed in a `Window` by the `ViewNavigator`.
...
[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/examples/ExampleView.java[tag=window2,indent=0]
----
...
*1. Using the provided `ViewWindowConfigurator`:* The `ViewNavigator` API provides a  `navigateInWindow(...)` method version which provides a link:{apidir}/com/holonplatform/vaadin/navigator/ViewWindowConfigurator.html[ViewWindowConfigurator^] API to configure the `Window` component into which the `View` will be displayed.

[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/examples/ExampleView.java[tag=window3,indent=0]
----
...
[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/examples/ExampleView.java[tag=window4,indent=0]
----
<1> Use the `@WindowView` annotation attributes to configure the view `Window`

*3. Using a `@ViewWindowConfiguration` annotated View method:* If a `View` class `public` method is annotated with the `@ViewWindowConfiguration` annotation and accepts a _single parameter_ of link:{apidir}/com/holonplatform/vaadin/navigator/ViewWindowConfigurator.html[ViewWindowConfigurator^] type, the method is called by the `ViewNavigator`, providing the `ViewWindowConfigurator` instance which can be used to configure the view `Window` component.

[source, java]
----
include::{examplesdir}/com/holonplatform/vaadin/examples/ExampleView.java[tag=window5,indent=0]
----
...
=== Obtain the current ViewNavigator
...
IMPORTANT: In order for the `View` authentication process to work properly, when the `@Authenticate` annotation is used on one or more specific `View` class (and not at `UI` level), a link:{apidir}/com/holonplatform/vaadin/navigator/ViewClassProvider.html[ViewClassProvider^] instance has to be bound to each registered `ViewProvider` to provide the `View` class which corresponds to a specific view name. The `ViewNavigator` builder API provides the `viewClassProvider(...)` method to configure the `ViewClassProvider`. When the <<VaadinSpring>> is enabled and the default `SpringViewNavigator` is used, a default `ViewClassProvider` is automatically configured, without needing to configure it explicitly.
