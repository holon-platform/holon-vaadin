...
To disable the Spring Security `@Secured` access control the `HolonVaadinSpringSecurityAutoConfiguration` class can be excluded:
...
This servlet allows automatic `com.vaadin.server.SessionInitListener` and `com.vaadin.server.SessionDestroyListener` registration if they are defined as Spring beans and made available in Spring context.
